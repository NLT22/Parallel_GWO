...
__device__ __forceinline__ float indexed_random_dev(int iter, int wid, int dim, int channel, uint64_t global_seed)
{
    uint64_t mix =
...
__global__ void init_pos_kernel(float* pos, int POP, int NDIM, uint64_t seed) {
    ...
    float r = indexed_random_dev(0, wolf, dim, 77, seed);
    pos[idx] = r; // in [0,1)
}
                                      ...
                                      float* __restrict__ sse_out)
{
    ...
    float partial = 0.0f;
    const float* cent = pos + (size_t)wolf * (size_t)(K * D);
    ...
    if (i < Ndata) {
        ...
        for (int k = 0; k < K; ++k) {
            const float* ck = cent + (size_t)k * (size_t)D;
            float dist = 0.0f;
            for (int d = 0; d < D; ++d) {
...
__global__ void reduce_top3_stage1(const float* fitness, int N,
...
__global__ void gather_leaders_kernel(const float* pos, int NDIM,
                                      const int* best3_idx,
...
__global__ void gwo_update_kernel(float* pos, int POP, int NDIM,
                                 const float* L0,
                                 const float* L1,
                                 const float* L2,
                                 float a,
                                 ...
                                 uint64_t seed)
{
    ...
    // j=0 (L0): channels 0,1
    float r1 = indexed_random_dev(iter, wolf, d, 0, seed);
    float r2 = indexed_random_dev(iter, wolf, d, 1, seed);
    float A1 = 2.0f * a * r1 - a;
    float C1 = 2.0f * r2;
    float D1 = fabsf(C1 * L0[d] - x);
    float X1 = L0[d] - A1 * D1;
    ...
    float C3 = 2.0f * r2;
    float D3 = fabsf(C3 * L2[d] - x);
    float X3 = L2[d] - A3 * D3;

    float newx = (X1 + X2 + X3) / 3.0f;
...
int main() {
    ...
    for (int POP : Pop_list) {
        ...
        // allocate
        float* d_pos = nullptr;   // [POP * NDIM]
        float* d_sse = nullptr;   // [POP]
        float* d_L0 = nullptr;    // [NDIM]
        float* d_L1 = nullptr;
        float* d_L2 = nullptr;
        int*    d_best3 = nullptr; // [3]

        size_t bytesPos = (size_t)POP * (size_t)NDIM * sizeof(float);
        size_t bytesSse = (size_t)POP * sizeof(float);
        size_t bytesVec = (size_t)NDIM * sizeof(float);
        ...
        for (int r = 1; r <= RUNS; ++r) {
            ...
            for (int iter = 0; iter < MAX_ITERS; ++iter) {
                ...
                // update
                float a = 2.0f * (1.0f - (float)iter / (float)MAX_ITERS);
