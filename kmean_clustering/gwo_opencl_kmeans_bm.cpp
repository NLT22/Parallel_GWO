...
// Lấy chuỗi thông tin của platform
...
// Lấy chuỗi thông tin của device
...
// Kiểm tra device có hỗ trợ fp64 không
...
// Kiểm tra OpenCL version >= 2.0
static bool device_opencl_ge_2(cl_device_id dev) {
    std::string v = get_device_str(dev, CL_DEVICE_VERSION);
    // parse "OpenCL X.Y"
    int major = 0, minor = 0;
    if (sscanf(v.c_str(), "OpenCL %d.%d", &major, &minor) == 2) {
        return (major > 2) || (major == 2);
    }
    // if parse fail, be strict
    return false;
}
...
// Làm tròn lên bội số của m
static size_t round_up(size_t x, size_t m) {
    return ((x + m - 1) / m) * m;
}

// OpenCL 2.0+ queue API ONLY
static cl_command_queue create_queue_new_api(cl_context ctx, cl_device_id dev) {
    cl_int err = CL_SUCCESS;
    const cl_queue_properties props[] = { 0 };
    cl_command_queue q = clCreateCommandQueueWithProperties(ctx, dev, props, &err);
    ocl_check(err, "clCreateCommandQueueWithProperties");
    if (!q) throw std::runtime_error("Failed to create command queue (need OpenCL 2.0+ runtime).");
    return q;
}

static std::string get_opencl_c_version(cl_device_id dev) {
    return get_device_str(dev, CL_DEVICE_OPENCL_C_VERSION);
}

static bool opencl_c_ge_20(cl_device_id dev) {
    std::string v = get_opencl_c_version(dev); // "OpenCL C X.Y ..."
    int major=0, minor=0;
    if (sscanf(v.c_str(), "OpenCL C %d.%d", &major, &minor) == 2) {
        return (major > 2) || (major == 2);
    }
    return false;
}
...
int main() {
    try {
        ...
        std::vector<int> LIMIT_list = {5000};
        std::vector<int> Pop_list = {512};
        ...
        std::string kernel_src = read_text_file("gwo_kmeans.cl");
        ...
        if (need_header) {
            csv << "impl,limit_train,Ndata,D,K,POP_SIZE,max_iters,runs,avg_ms,best_sse,"
                   "platform,device,ocl_version,fp_mode,local_x,nGroupsX\n";
        }

        // enumerate platforms (làm 1 lần)
        cl_uint nplat = 0;
        ...
        int ran_count_total = 0;

        // ===================== LOOP over LIMIT_list =====================
        for (int LIMIT_TRAIN : LIMIT_list) {
            // Load MNIST theo LIMIT hiện tại
            ...
            std::cout << "\n====================================================\n";
            std::cout << "LIMIT_TRAIN=" << LIMIT_TRAIN
                      << " | Loaded MNIST: N=" << Ndata << ", D=" << D
                      << " (" << train.rows << "x" << train.cols
                      << "), NDIM=" << NDIM << "\n";

            int ran_count_this_limit = 0;
            ...
            for (cl_uint pi = 0; pi < nplat; ++pi) {
                ...
                for (cl_uint di = 0; di < ndev; ++di) {
                    ...
                    if (!device_opencl_ge_2(dev)) {
                        std::cout << "Skip: OpenCL < 2.0 (need queue new API)\n";
                        continue;
                    }
                    ...
                    // Policy:
                    // - Intel Iris: force FP32
                    // - others: use FP64 if supported
                    bool force_fp32 =
                        contains_icase(plat_name, "Intel(R) OpenCL HD Graphics") &&
                        contains_icase(dev_name, "Iris");
                    bool use_fp64 = supports_fp64 && !force_fp32;
                    ...
                    try {
                        q = create_queue_new_api(ctx, dev);
                    ...
                    // Build options
                    bool c20 = opencl_c_ge_20(dev);
                    std::string options =
                        std::string(c20 ? "-cl-std=CL2.0 " : "-cl-std=CL1.2 ")
                    + "-D K_CONST=10 -D USE_FP64=" + (use_fp64 ? "1" : "0");
                    ...
                    // nGroupsX phụ thuộc Ndata nên phải tính theo LIMIT hiện tại
                    int nGroupsX = (Ndata + (int)LOCAL_X - 1) / (int)LOCAL_X;

                    const size_t realSize = use_fp64 ? sizeof(double) : sizeof(float);

                    // X buffer (phụ thuộc Ndata nên tạo theo LIMIT hiện tại)
                    cl_int em = 0;
                    ...
                    for (int POP : Pop_list) {
                        std::cout << "--------------------------------------------\n";
                        std::cout << "OpenCL FULL | LIMIT=" << LIMIT_TRAIN
                                  << " | POP=" << POP
                                  << " | ITERS=" << MAX_ITERS
                                  << " | nGroupsX=" << nGroupsX
                        ...
                        cl_mem dCand = clCreateBuffer(ctx, CL_MEM_READ_WRITE,
                            (size_t)N3 * sizeof(double) * 2, nullptr, &em);
                        ...
                        for (int r = 1; r <= RUNS; ++r) {
                            ...
                            for (int iter = 0; iter < MAX_ITERS; ++iter) {
                                ...
                                // reduce1
                                {
                                    ...
                                    size_t localBytes = sizeof(double) * 2 * LOCAL_X;
                                ...
                                // update
                                {
                                    double a_host = 2.0 * (1.0 - (double)iter / (double)MAX_ITERS);
                                    cl_ulong seed = (cl_ulong)SEED;
                                    ...
                                    if (use_fp64) {
                                        double a = a_host;
                                        ocl_check(clSetKernelArg(k_upd, 6, sizeof(double), &a), "arg upd 6 fp64");
                                    } else {
                            ...
                            if (use_fp64) {
                                ...
                                ocl_check(clEnqueueReadBuffer(q, dSSE, CL_TRUE,
                                    (size_t)best3[0] * sizeof(double), sizeof(double), &v, 0, nullptr, nullptr),
                                    "read sse(best) fp64");
                                bestSSE = v;
                            } else {
                                float v = 0.0f;
                    ...
                    ran_count_this_limit++;
                    ran_count_total++;
                }
            }

            if (ran_count_this_limit == 0) {
                std::cout << "No target devices found / runnable for LIMIT=" << LIMIT_TRAIN
                          << " (OpenCL >=2.0 required).\n";
            }
        }

        if (ran_count_total == 0) {
            std::cout << "\nNo target devices found / runnable at all (OpenCL >=2.0 required).\n";
        }
