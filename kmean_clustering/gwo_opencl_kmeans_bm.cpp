    ...
    try {
        ...
        for (cl_uint pi = 0; pi < nplat; ++pi) {
            ...
            for (cl_uint di = 0; di < ndev; ++di) {
                ...
                bool force_fp32 = contains_icase(plat_name, "Intel(R) OpenCL HD Graphics") && contains_icase(dev_name, "Iris");
                ...
                // Force CL2.0 kernel dialect (most runtimes accept this; if not, remove -cl-std)
                std::string options = std::string("-cl-std=CL2.0 -D K_CONST=10 -D USE_FP64=") + (use_fp64 ? "1" : "0");
                ...
                for (int POP : Pop_list) {
                    ...
                    cl_mem dCand = clCreateBuffer(ctx, CL_MEM_READ_WRITE,
                        (size_t)N3 * sizeof(double) * 2, nullptr, &em); // enough for PairFI padding
                    ...
                    for (int r = 1; r <= RUNS; ++r) {
                        ...
                        for (int iter = 0; iter < MAX_ITERS; ++iter) {
                            // sse_partial (2D)
                            {
                                ...
                                // dynamic local smem (LOCAL_X entries)
                            ...
                            // reduce1
                            {
                                ...
                                // local arrays of PairFI (3 arrays)
                                size_t localBytes = sizeof(double) * 2 * LOCAL_X; // enough even if PairFI padded
        ...
        if (ran_count == 0) {
            std::cout << "\nNo target devices found / runnable (OpenCL >=2.0 required).\n";
        }
