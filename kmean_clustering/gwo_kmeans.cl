...
#if defined(USE_FP64) && (USE_FP64==1)
...
#if defined(USE_FP64) && (USE_FP64==1)
    // Match CPU: (r>>11)*2^-53
    // Use hex-float to avoid "double constant" issues.
    // 0x1.0p-53 is double; in FP64 mode that's fine.
    return (real)((double)(r >> 11) * 0x1.0p-53);
#else
    // FP32 mode (no double allowed):
    // Take top 24 random bits -> uniform float in [0,1)
    // u = (r >> 40) / 2^24
    uint v = (uint)(r >> 40); // 24-bit
    return (real)((float)v * 0x1.0p-24f);
#endif
}
